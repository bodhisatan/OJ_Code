...
*          2017-2018    revise by Techiah
*          2018-*       revise by Bodhisatan
**********************************************************************************************************/
/******catalogue***********
简单算法与STL！！！！！
    STL
    输入输出外挂
	大数处理
		大数加法，减法，乘法, 比较, 交大版结构体高精度大板，JAVA
	回文数
	拓扑排序
...
BigNumber operator *(const BigNumber &a,const long long &k)
{
	BigNumber c;
	c.d[0] = a.d[0];
	long long i, x = 0;
...
struct matx              //构造矩阵 & 重载乘法 
{
	...
	matx operator* (matx b)
	{
		...
		for(int j=0; j<2; j++)
		{
			...
			for(int k=0; k<2; k++)
			{
				tmp = (num[i][k] * b.num[k][j]) % mod;
				ans.num[i][j] = (ans.num[i][j] + tmp) % mod;
			}
		}
		return ans;
	}
...
matx q_pow(matx a,LL N)                //矩阵快速幂 
{
	matx ans;
	memset(ans.num,0,sizeof(ans.num));
	for(int i=0;i<7;i++)  ans.num[i][i]=1;
	while(N)
	{
		if(N%2==1)  ans=ans*a;
		a=a*a;
